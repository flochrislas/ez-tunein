...
- **`lib/audio_handler.dart`** — `EzAudioHandler` (the single `audio_service`
  media-session handler), the `AudioModeDriver` interface both pages implement,
  the `PlaybackMode` enum, and the `WifiLock` MethodChannel wrapper. UI-free.
...
- **Song recording is a raw-byte dump — do NOT add an audio codec.** Icecast/Shoutcast streams are *already* compressed (MP3/AAC), so recording just writes those bytes verbatim — lossless, instant, zero extra dependencies. `IcyReader` already reads every audio byte to reach the metadata; it forwards them (batched, metadata stripped) via the `onAudio` callback. `StreamRecorder` (`lib/stream_recorder.dart`) buffers the current track to a temp file (`getTemporaryDirectory()`), capped at `rec_buffer_mb` MB (oldest bytes trimmed) until the user hits **Record** (`arm`), then keeps appending; on the next track change (`onTrackChanged`, driven by the ICY title) or stop/station-switch (`onStreamStopped`) it finalizes the buffer to `<output>/Artist - Title.<ext>` (extension from the stream's `Content-Type`). One song at a time; tapping Record again `cancel`s. The record button is hidden when `rec_buffering` is off (no buffer ⇒ nothing to record). **Never** re-encode to change bitrate (would need an ffmpeg-class dep, against the minimalist goal) — the rate is always the stream's own. Track-change handling dedups on `_lastRecTitle` and skips the *first* title of a session so the initial song's buffer isn't reset. **The buffer is a segmented ring** (`_Segment` files, size ≈ ⅛ of the cap, 1–16 MB): `addAudio` appends to the active segment, rolls a new one when it fills, and — until armed — drops the oldest *whole* segment (an O(1) delete, no read) once the rest still covers the cap. This deliberately replaced a single-file design that rewrote ~cap bytes to trim (a recurrent multi-MB RAM spike + UI hitch). **Don't reintroduce a read-all-and-rewrite trim in `addAudio`.** Finalize concatenates the live segments (streamed in 1 MB chunks; one-segment songs take a rename fast path). `rec_buffer_mb` is capped at `_recBufferMbMax` (128 MB; old larger saved values are clamped on read) to bound disk use + the finalize copy. **The async lifecycle ops (`arm`/`startBuffering`/`onTrackChanged`/`onStreamStopped`/`dispose`) are serialized through `_runExclusive`** so a metadata-driven finalize can't interleave with `_stop`'s finalize and corrupt the shared segment state; `addAudio` is synchronous and unqueued — it no-ops whenever there's no open active segment (i.e. during teardown). **`arm` is queued too and returns `Future<bool>`** so a Record tap landing during an in-flight finalize/`startBuffering` (e.g. right at a track change) *waits* for the fresh buffer and arms it instead of no-opping while `_raf` is momentarily null (C2); the player only sets `_recording` when `arm` returns true. **Finalize returns a `FinalizeResult` record (`{path, error}`)** — `path` set ⇒ saved, both null ⇒ nothing was armed, `error` set ⇒ finalize failed (the player snacks `Recording failed: …` and the partial output file is deleted) rather than an ambiguous silent `null` (C1/C7). Public ops call private bodies (`_startBuffering`, etc.) to avoid re-entrant deadlock. **Segments live in a private per-process temp subdir** (`(await bufferDirResolver()).createTemp('ez_tunein_')` → unpredictable name, 0700 on POSIX) rather than fixed names in the shared temp dir — this closes a Linux `/tmp` symlink-truncation vector and stops two app instances corrupting each other's buffers (S3/C6); the old shared-dir sweep is gone. `dispose` deletes the private dir; the desktop `exit(0)` close path calls `_recorder.dispose()` explicitly (it skips `State.dispose`), so only a hard crash leaks one dir (left to OS temp cleanup). The ring buffer's **base** temp dir + segment size are injectable (`bufferDirResolver` / `segmentBytesOverride`), and the private dir is exposed via `bufferDir`, so it's unit-tested (`test/stream_recorder_test.dart`). See `doc/implementation-notes.md`.
- **Background playback + recording survive screen-off on Android via `audio_service`** (a true Android `MediaSession` — `audio_service` 0.18.x wraps `MediaSessionCompat`/`NotificationCompat`, not Media3 — with a rich media-card notification, lock-screen controls, and Bluetooth/headset/car AVRCP). It replaced `flutter_foreground_task`. Both playback and the recording/metadata path (`IcyReader`'s socket loop) run on the main isolate; with the screen off Android's Doze would freeze them, so `audio_service`'s `mediaPlayback` **foreground service** + wake lock (plus a **native Wi-Fi lock**, see below) hold the process alive. **Architecture:** one singleton `EzAudioHandler extends BaseAudioHandler` (in `lib/audio_handler.dart`) owns the session but **delegates** to whichever page is driving audio via an `AudioModeDriver` interface (`driverPlay/Pause/Stop/Seek/SkipNext`). The players + their logic stay in the State classes. A page calls `audioHandler.attach(mode, this)` + `publishRadio(...)` / `publishRecording(...)` / `updateRecordingPosition(...)` to push `mediaItem` + `playbackState`; `detach(this)` clears the session **only if the caller is still the active driver** (an `identical` guard — the key handoff-race fix, replacing the old `rec_`-prefix hack). Transport events (notification / lock-screen / Bluetooth / car) come back through the handler's `play/pause/stop/seek/skipToNext` overrides → the active driver. This is what lets the app keep **two** `AudioPlayer`s (radio + recordings) behind one session — **not** `just_audio_background`, which allows only one `AudioPlayer` app-wide and would crash `_RecordingsPage`'s separate player. **Radio driver** (`_PlayerPageState`): `_publishRadio()` (station name + now-playing/recording text, no duration, Play/Pause + Stop controls) replaces the old `_syncPlaybackService`; it's called from `_play`, `_handleTrackChange`, `_toggleRecord`/`_saveManualRecording`, the buffering-off branch of `_applyRecordingPrefs`; `_stop` calls `detach`. **Radio pause is non-destructive** (`_pauseRadio` → `_player.pause()` only): the metadata/recording socket keeps running so a Bluetooth/car pause never kills an in-progress recording; `driverPlay` resumes it (`_resumeRadio`, tracked by `_paused`). A full **Stop** detaches the session entirely (no lingering notification), so there's no Play-after-Stop reconnect — the user restarts a station from the app. The in-app **Mute** button stays (volume→0, instant); the media card uses standard Play/Pause (pause also silences) rather than a custom mute action. **Recordings driver** (`_RecordingsPageState`): `_publishRecording()` (label + duration + Play/Pause + Skip + Stop) + a throttled `positionStream`→`updateRecordingPosition` feeds a working lock-screen scrubber; auto-advance/shuffle stay in the State. **Swipe-away saves:** `EzAudioHandler.onTaskRemoved` calls the active driver's `driverStop` (the handler is on the main isolate, so the recorder is reachable) → an in-progress recording is finalized before the process dies. **Init:** `main()` calls `AudioService.init(builder: () => EzAudioHandler(), config: …)` **only on Android/iOS**; on desktop it plain-constructs `EzAudioHandler()` (no native session), so the `exit(0)` close path is untouched; `_publishRadio`/`_publishRecording` early-return off mobile. Album art is a `file://` copy of `assets/icon/app_icon_256.png` (cached once by `_prepareArtUri`; audio_service needs a loadable bitmap, not an asset path), used for both modes. Config: `androidStopForegroundOnPause: false` + `androidNotificationOngoing: false` (keeps the service alive while paused so a paused radio still records; ongoing must be false or audio_service asserts), small icon `drawable/ic_stat_media`. Manifest: `FOREGROUND_SERVICE` + `_MEDIA_PLAYBACK` + `POST_NOTIFICATIONS` + `WAKE_LOCK` + `ACCESS_WIFI_STATE`/`CHANGE_WIFI_STATE`, the `com.ryanheise.audioservice.AudioService` service (with `MediaBrowserService` intent-filter) + `MediaButtonReceiver`. `MainActivity` extends `AudioServiceActivity` (media-button routing). The UI also glows a red neon border on the now-playing box while `_recording`. (This doesn't change the ICY-metadata limitation: stations that announce a title early can still make a recording cut feel early.)
- **Native Wi-Fi lock (Android).** `audio_service` holds a wake lock but **not** a `WifiManager.WifiLock`; the ICY metadata/recording socket (separate from ExoPlayer) needs the Wi-Fi radio kept awake under Doze to keep feeding the recorder screen-off. A tiny `MethodChannel('ez_tunein/wifi_lock')` in `MainActivity.kt` acquires/releases a `WIFI_MODE_FULL_HIGH_PERF` lock; the Dart `WifiLock` wrapper (in `audio_handler.dart`) is called from `EzAudioHandler.attach` (acquire) / `detach` (release). Best-effort, Android-only. This preserves the screen-off recording-finalize guarantee that `flutter_foreground_task`'s Wi-Fi lock used to provide — **verify on real hardware** (`adb shell dumpsys deviceidle force-idle`).
- **Settings** (`_SettingsPage`, app-bar gear icon — on the player **and** the recordings view) is one combined screen: **Appearance** (accent color) on top, then the recording prefs. **Accent color:** the Material 3 seed (`colorSchemeSeed`) lives in a top-level `ValueNotifier<Color> accentColor` (default `_defaultAccentValue` = teal, persisted as `accent_color`); `RadioApp` wraps `MaterialApp` in a `ValueListenableBuilder` on it, so picking a color re-themes the whole app **live**. The settings page offers preset swatches + R/G/B sliders (`Color.fromARGB` / `.toARGB32()` / `.r`·`.g`·`.b` — the non-deprecated color API; don't use `.value`/`.red`). `main()` loads the saved accent before `runApp`; `_TrackListPage`'s forced-dark `Theme` reads `accentColor.value` too. The recording prefs are written directly; the player re-applies them on pop via `_applyRecordingPrefs` (which also starts/stops live buffering on a toggle). The buffer-size slider (5–`_recBufferMbMax` MB) has a `_bufferGuide` caption showing the MB cost of a minute of MP3 at 128/256/320 kbps and how many minutes the chosen size rewinds, so the size choice is meaningful. Output folder defaults to **`<Downloads>/EZ-TuneIn`** on desktop (`getDownloadsDirectory()` + `recSubdirName`; picked via `file_picker`'s `getDirectoryPath`), falling back to `<Documents>/EZ-TuneIn` if the platform reports no Downloads dir (the subfolder applies to **both** desktop bases — Documents is shared too); on Android it always uses the app-private app folder, bare (arbitrary-folder writes need SAF, not done). **The subfolder is deliberate** — `listRecordings()` lists *every* audio file in the output folder (and the library's per-row Delete acts on them), so recording straight into a shared Downloads folder mixed in unrelated mp3s. It's created lazily (recorder's finalize, or the library's open-folder button). The settings row shows the **resolved** default via `defaultRecordingsDir()` (the shared fall-through of `recordingsDir()`) rather than a hard-coded `Downloads/…` string. No migration was done: users who recorded before this change keep those files loose in Downloads.
- **Recordings library** (`_RecordingsPage`, app-bar `library_music` icon, between Saved tracks and Settings) lists/plays the audio files in the recordings folder. It owns its **own** `AudioPlayer` (separate from the radio's `_player`), applies the saved `volume`, and **stops playback on dispose** (leaving the view). Starting a song calls the passed-in `stopRadio` (the player's `_stop`) **once** so the two never sound together. Track names are parsed from filenames via `splitArtistTitle`. `rec_never_stops` auto-advances on `ProcessingState.completed`; `rec_randomize` picks the next file via `Random()` (affects both auto-advance and the skip button). Folder is located by the shared top-level `recordingsDir()` (the recorder's `outputDirResolver` and `listRecordings()` both use it); `isAudioFile` gates the listed extensions. Export writes an `artist,title` CSV via the same `file_picker` `saveFile` flow as `_exportStations`. Per-row overflow menu offers **Delete** (`_deleteFile`, confirm dialog, frees space) and **Share/move** (`_shareFile` via `share_plus`, mobile only); a desktop-only **folder** icon (`_openFolder`) reveals the dir. **Don't** await `_player.play()` for these files either — completion is observed on `playerStateStream` (and auto-advance is deferred off that callback to avoid a re-entrant parked-playback bug); duration comes from `durationStream`, not `setFilePath`. On Android it gets the **same `audio_service` media-session treatment** as the radio (background playback with the screen off), driving the shared `EzAudioHandler` as the `recordings` mode via `_publishRecording()` (off the player-state stream) with richer controls suited to finite files — **Play/Pause + Skip + Stop** + a working lock-screen scrubber (fed by a throttled `positionStream`→`updateRecordingPosition`). Implements `AudioModeDriver` (`driverPlay/Pause`→`_player.play/pause`, `driverStop`→`_stopPlayback`, `driverSeek`→`_player.seek`, `driverSkipNext`→`_skip`); `attach`es on first play and `detach`es in `dispose`. The single-active-driver `identical` guard means no clash with the radio page (the radio is stopped whenever recordings playback is active).
...
- **Online station search** (`_addStation` → `_StationSearchPage`, backed by `lib/radio_browser.dart`). The **+** action no longer opens `_StationDialog` directly — it pushes a full search page (keyword `TextField` + results `ListView` of favicon/name/metadata rows with checkboxes + an "Add N stations" button) that pops a `List<Station>`; `_addStation` merges them with the **same `Set.add` dedup as `_importStations`**. The page's app-bar pencil opens the classic manual `_StationDialog` and pops its single result through that same path. Backed by the free, **no-key Radio Browser API** (`api.radio-browser.info`) via a **new `dart:io HttpClient`** (no `package:http` — matches `IcyReader`), with round-robin `all.api.…` + named-mirror fallbacks and a `ez_tunein/<version>` User-Agent (the API asks for a "speaking" agent). **Prefer `url_resolved`** (the API's already-unwrapped **direct** stream URL) over `url` — that's exactly the direct-URL format the app requires, so search results need no playlist unwrapping. `parseRadioBrowserStations` is pure + unit-tested; `searchRadioBrowser` (network) isn't. `existingUrls` marks already-added stations "Added"/disabled.
...
- **Desktop close-while-playing crash.** Closing the window mid-stream races media_kit's native shutdown and **segfaults** (the GTK engine also logs `FlutterEngineRemoveView … implicit view cannot be removed`); it only happens with an active audio engine (reproduced ~60% of closes while playing, 0% when idle). Fix: `_PlayerPageState` calls `windowManager.setPreventClose(true)` on desktop (in `initState`) and its `onWindowClose` (`WindowListener`) finalizes any recording via `_stop()` then **`exit(0)`** — hard-exiting *before* the racy engine/plugin teardown. `exit(0)` (not `windowManager.destroy()`) is deliberate: it also covers `_RecordingsPage`'s **separate** `AudioPlayer`, which the player State can't reach. `_closing` guards re-entry. Don't replace this with a graceful destroy.
- **App launcher icons** come from **one vector source**, `assets/icon/icon.svg` (an Inkscape drawing: red disc + white music note + two red broadcast arcs on a near-black radial gradient). It is the *only* hand-edited art. [`script/make_icons.py`](script/make_icons.py) rasterizes it into the five generated PNGs in `assets/icon/` — `icon.png` (1024² tile, Windows + Android legacy), `app_icon_256.png` (Linux GTK + media-session art), `icon_macos.png` (Apple's template), `icon_background.png` (the bare gradient) and `icon_adaptive_foreground.png` (the logo alone on transparency, scaled by `FILL_ADAPTIVE`) — using whichever of `rsvg-convert`/Inkscape/headless-Chrome is on `PATH`. **Corner shape is per-platform, because only Android masks for us:** the two adaptive layers stay **full-bleed squares** (rounding them would be clipped by the launcher mask), `icon.png`/`app_icon_256.png` get a subtle `CORNER_RADIUS` (12%) tile with transparent corners, and macOS gets its own `icon_macos.png` — Apple's post-Big-Sur template, a rounder tile (`MACOS_RADIUS` 22.5% *of the tile*) **inset** to `MACOS_INSET` (80.5%) of the canvas, since the Dock does no masking and full-bleed art sits larger than its neighbours. `pubspec.yaml` points `macos: image_path:` at it; `image_path:`/`windows:` keep `icon.png`. **The two adaptive halves are derived from the SVG in memory, by element id** (`layer5` = background rect; `rect27`/`rect27-0` = the black bars that fake the arcs by painting over a full circle — the foreground deletes them and clips the ring `path8-4` to the band between them instead, so no black bars land on the transparent foreground). Restructuring the drawing changes those ids ⇒ the script **fails loudly**; update its constants. `FILL_ADAPTIVE` is 0.96 because `flutter_launcher_icons` *also* insets the foreground by 16% — net ≈0.64 of the canvas, inside Android's 72/108 visible circle. The `pubspec.yaml` `adaptive_icon_foreground` points at the generated `icon_adaptive_foreground.png`, `adaptive_icon_background` at `icon_background.png`. Config is the `flutter_launcher_icons:` block in `pubspec.yaml`; **regenerate with `python3 script/make_icons.py` then `~/flutter/bin/dart run flutter_launcher_icons`** after changing the art (it rewrites Android mipmaps/adaptive drawables, `windows/runner/resources/app_icon.ico`, and the macOS `AppIcon.appiconset`). **Linux isn't covered by the tool** — the GTK window/taskbar icon is set natively in `linux/runner/my_application.cc` from the bundled `assets/icon/app_icon_256.png` (declared under `flutter: assets:`, so it ships in `data/flutter_assets/…` next to the executable; the runner reads it via `/proc/self/exe`). Native change ⇒ needs a full `flutter build linux`. No iOS/web icons (those folders don't exist). **Full how-to (regenerate + every tweakable knob) in [`doc/app-icon.md`](doc/app-icon.md).**
...
guide); the recordings library reads real files from `~/Downloads/EZ-TuneIn` (the
default recording folder) — drop a couple of `Artist - Title.mp3` files there if it's empty,
...
- Android **recording folder** (recordings save to the app folder; picking an
  ...
  desktop saves to `Downloads/EZ-TuneIn` / a chosen folder).
